...
int ensure_path_mounted(const char* path) {
    ...
    if (strcmp(v->fs_type, "yaffs2") == 0) {
		...
		result = mount(v->device, v->mount_point, "ntfs",
						   MS_NOATIME | MS_NODEV | MS_NODIRATIME, "");
		if (result == 0) return 0;
        ...
        if (v->device2) {
            ...
            LOGW("failed to mount %s;trying mount %s to ntfs\n", v->device, v->device2);
			result = mount(v->device2, v->mount_point, "ntfs",
							   MS_NOATIME | MS_NODEV | MS_NODIRATIME, "");
			if (result == 0) return 0;
        }
...
int format_volume(const char* volume) {
    ...
    if (strcmp(v->fs_type, "ext4") == 0) {
        int result = rk_make_ext4fs(v->device, v->length, volume);
        if (result != 0) {
    ...
    if (strcmp(v->fs_type, "ext3") == 0) {
        int result = rk_make_ext3fs(v->device);
        if (result != 0) {
    ...
    if (strcmp(v->fs_type, "ntfs") == 0) {
		...
		property_get("UserVolumeLabel", volume_label, "");
		LOGI("VolumeLabel: %s\n", volume_label);
		int result = make_ntfs(v->device,volume_label);
		if (result != 0) {
