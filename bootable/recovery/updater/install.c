...
Value* FormatFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (strcmp(partition_type, "MTD") == 0) {
    	...
    	mount(v->device, v->mount_point, v->fs_type,
		...
		if (status != 0) {
			fprintf(stderr, "format_volume: make_ntfs failed on %s\n", location);
			result = strdup("");
			goto done;
		 }
		result = location;
...
Value* WriteRawImageFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if(emmcEnabled) {
    	...
    	FILE *dest_partition = fopen(v->device, "wb");
    	if(dest_partition == NULL) {
    		fprintf(stderr, "%s: no emmc partition named \"%s\"\n", name, v->device);
			result = strdup("");
			goto done;
    	}
    	...
    	if (contents->type == VAL_STRING) {
			...
			if (!success) {
				fprintf(stderr, "emmc data write to %s failed: %s\n",
